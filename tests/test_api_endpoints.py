...
class TestAPIEndpoints(unittest.TestCase):
    ...
    def test_open_device(self, mock_open):
        response = self.client.post("/devices/5/open")
        self.assertEqual(response.status_code, 200)
    ...
    def test_open_hold_device(self, mock_hold):
        response = self.client.post("/devices/10/open_hold")
        self.assertEqual(response.status_code, 200)
    ...
    def test_reset_device(self, mock_reset):
        response = self.client.post("/devices/15/reset")
        self.assertEqual(response.status_code, 200)
    ...
    def test_device_reset(self, mock_device_reset):
        response = self.client.post("/devices/20/device_reset")
        self.assertEqual(response.status_code, 200)
    ...
    def test_factory_reset(self, mock_factory_reset):
        response = self.client.post("/devices/25/factory_reset")
        self.assertEqual(response.status_code, 200)
